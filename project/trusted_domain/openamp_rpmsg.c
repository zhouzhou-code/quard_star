 ...
 *   - notify 经 RV-Mailbox：FreeRTOS->Linux 写 to-Linux SET；poll 清 to-RTOS CLEAR
 *
...
/* RV-Mailbox doorbell 偏移（见 hwspecs.h）：
 *   FreeRTOS->Linux: 写 to-Linux SET (RVMB_TL_SET=0x04)
 *   FreeRTOS 应答 Linux 的 kick: 写 to-RTOS CLEAR (REG_RTOS_ACK=0x108) */
#define MB_NOTIFY_LINUX   RVMB_TL_SET     /* 0x04 */
#define MB_ACK_RTOS       REG_RTOS_ACK    /* 0x108 */
#define MB_DBELL          RVMB_RPMSG_DBELL
...
/* ---- remoteproc ops：mmap(恒等映射) + notify(戳 RV-Mailbox) ---- */
...
static int rproc_notify_op(struct remoteproc *rproc, uint32_t id)
{
    (void)rproc;
    (void)id;
    /* FreeRTOS -> Linux: 写 RV-Mailbox to-Linux SET 触发 IRQ50 */
    *(volatile uint32_t *)(MAILBOX_BASE_ADDR + MB_NOTIFY_LINUX) = MB_DBELL;
...
void openamp_rpmsg_poll(void)
{
    if (!g_inited)
        return;
    /* 清 Linux 的 to-RTOS 门铃(应答)，再处理 RX virtqueue */
    *(volatile uint32_t *)(MAILBOX_BASE_ADDR + MB_ACK_RTOS) = MB_DBELL;
    __asm__ volatile("fence ow,ow" ::: "memory");
